...
export function pageForPath(path:string):ReactNode{
 const seg=path.split('/').filter(Boolean);let c:ReactNode;
...
function AuthGate(){
 const{session,loading}=useAuth();const{path}=useRouter();const isMobile=useIsMobile();
 ...
 if(loading)return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="flex flex-col items-center gap-3"><Spinner size={28}/><p className="text-sm text-gray-500">Loading LytHouse…</p></div></div>;
 {const seg=path.split('/').filter(Boolean);if(seg[0]==='invite'&&seg[1])return <AcceptInvitePage token={seg[1]}/>;}
 if(!session){if(path==='/signup'||path==='/trial'||path==='/free-trial')return <Suspense fallback={<PageFallback/>}><FreeTrialPage/></Suspense>;if(path==='/demo'||path==='/book-demo')return <Suspense fallback={<PageFallback/>}><BookDemoPage/></Suspense>;if(path==='/docs')return <Suspense fallback={<PageFallback/>}><DocsPage/></Suspense>;if(path==='/plans'||path==='/pricing')return <div className="min-h-screen bg-[#fbfaff]"><div className="mx-auto max-w-6xl px-5 py-12"><Suspense fallback={<PageFallback/>}><PlansPage/></Suspense></div></div>;const authPaths=['/signin','/auth','/login'];if(authPaths.includes(path))return <AuthPage initialMode='signin'/>;return <LandingPage/>;}
