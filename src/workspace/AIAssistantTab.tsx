  ...
  return<div className="space-y-1">
    {lines.map((line,i)=>{
      ...
      if(line.includes('|')&&line.trim().startsWith('|')){
        ...
        return<div key={i} className={`grid text-xs gap-2 ${isHeader?'font-bold text-gray-500 uppercase tracking-wide border-b border-[#18181b] pb-1':'text-navy-700 py-0.5'}`} style={{gridTemplateColumns:`repeat(${cells.length},1fr)`}}>
  ...
  return(
    <div className="flex flex-col" style={{height:"calc(100vh - 300px)",minHeight:480}}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 pr-1">
        {messages.length===0&&(
          <div className="py-6">
            <div className="mb-5 rounded-lg border border-[#18181b] bg-gray-50 px-4 py-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {QUICK_PROMPTS.map((p,i)=>(
                <button key={i} onClick={()=>handleSend(p.text)} className="flex items-center gap-3 p-3 rounded-lg border border-[#18181b] hover:border-brand-300 hover:bg-brand-50 transition-all text-left group">
        ...
        {messages.map((m,i)=>(
          <div key={i} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}>
            <div className={`max-w-[88%] ${m.role==='user'?'bg-navy-900 text-white rounded-2xl rounded-br-sm':'bg-white border border-[#18181b] rounded-2xl rounded-bl-sm'} p-4 shadow-sm`}>
              {m.role==='assistant'&&(
        ...
        {loading&&(
          <div className="flex justify-start">
            <div className="bg-white border border-[#18181b] rounded-2xl rounded-bl-sm p-4 shadow-sm">
