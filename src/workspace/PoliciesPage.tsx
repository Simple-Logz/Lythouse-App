      ...
      {creating&&(
        <div className="card border-2 border-brand-300 bg-brand-50">
          ...
          <div className="space-y-2 mb-4">
            {newRules.map(rule=>(
              <div key={rule.id} className={`flex items-start justify-between gap-3 rounded-lg border px-3 py-2.5 transition-all ${rule.required?'border-brand-300 bg-white':'border-[#d4d4d8] bg-gray-50 opacity-60'}`}>
      ...
      {policies.length===0&&!creating?(
        ...
        <div className="space-y-3">
          {policies.map(policy=>{
            ...
            return(
              <div key={policy.id} className={`card p-0 overflow-hidden border-2 ${policy.enabled?'border-brand-200':'border-[#d4d4d8] opacity-60'}`}>
                <div className={`flex items-center gap-3 px-5 py-4 cursor-pointer ${policy.enabled?'bg-brand-50/50':'bg-gray-50'}`} onClick={()=>setExpanded(isOpen?null:policy.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`chip text-xs border capitalize ${policy.environment==='production'?'bg-red-50 text-red-600 border-red-200':policy.environment==='staging'?'bg-amber-50 text-amber-600 border-amber-200':'bg-gray-100 text-gray-600 border-[#d4d4d8]'}`}>{policy.environment}</span>
                      {policy.enabled?<span className="chip bg-green-50 text-green-700 border border-green-200 text-xs"><Check size={10}/>Active</span>:<span className="chip bg-gray-100 text-gray-500 border border-[#d4d4d8] text-xs">Disabled</span>}
                    </div>
                  ...
                  <div className="border-t border-gray-100 px-5 py-4">
                    ...
                    <div className="space-y-2">
                      {policy.rules.map(rule=>(
                        <div key={rule.id} className={`flex items-start justify-between gap-3 rounded-lg border px-3 py-2.5 transition-all ${rule.required?'border-brand-200 bg-brand-50':'border-[#d4d4d8] bg-gray-50'}`}>
