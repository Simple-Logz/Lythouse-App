...
const BIZ_META:Record<BusinessSeverity,{label:string;color:string;bg:string;border:string;icon:typeof XCircle}> = {
  ...
  informational:{label:'Informational',color:'text-gray-600',bg:'bg-gray-50',border:'border-[#d4d4d8]',icon:Eye},
};
    ...
    <div className={`rounded-xl border-2 overflow-hidden transition-all ${meta.border} ${f.status==='resolved'?'opacity-50':''}`}>
      ...
      <div className={`flex items-start gap-3 px-4 py-3.5 cursor-pointer ${meta.bg} hover:brightness-[0.98]`} onClick={()=>setOpen(o=>!o)}>
        ...
        <div className="flex items-center gap-1.5 shrink-0" onClick={e=>e.stopPropagation()}>
          ...
          {f.status==='open'&&<button onClick={()=>onStatusChange(f.id,'ignored')} className="px-2 py-1 text-xs text-gray-500 border border-[#d4d4d8] rounded-lg hover:bg-gray-100">Ignore</button>}
        </div>
      </div>
      ...
      {open&&(
        <div className="bg-white border-t border-gray-100 px-4 py-4 space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-1 border-t border-gray-100">
            ...
            <button onClick={createTicket} disabled={state.loadingTicket} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#d4d4d8] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100 transition-colors disabled:opacity-50">
              {state.loadingTicket?<Loader2 size={12} className="animate-spin"/>:<Ticket size={12}/>}Create Ticket
            </button>
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#d4d4d8] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100 transition-colors">
              <GitPullRequest size={12}/>Create PR
            </button>
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#d4d4d8] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100 transition-colors">
              <MessageSquare size={12}/>Notify Slack
            </button>
            {f.file_path&&<button onClick={()=>onOpenFile(f.file_path!,f.line??undefined)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#d4d4d8] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100 transition-colors">
              <FileCode size={12}/>Open File
            </button>}
            {f.status!=='open'&&<button onClick={()=>onStatusChange(f.id,'open')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#d4d4d8] text-gray-500 text-xs hover:bg-gray-50 ml-auto">
