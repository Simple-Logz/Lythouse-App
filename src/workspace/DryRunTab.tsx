...
{active&&phases.indexOf(phase)===i?<Loader2 size={16} className="animate-spin text-brand-600"/>:active?<CheckCircle2 size={16} className="text-brand-500"/>:<div className="w-4 h-4 rounded-full border-2 border-gray-200"/>}
...
return<button key={env} onClick={()=>setEnvironment(env)} className={'flex items-center gap-2 rounded-xl border-2 px-4 py-3 transition-all '+(a?'border-brand-500 bg-brand-50':'border-gray-200 hover:border-gray-300')}>
...
return<div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
...
return<div className="rounded-xl border border-gray-200 bg-gray-50 p-3"><div className="flex items-center gap-1.5 mb-1 text-gray-400">{icon}<span className="text-xs font-medium">{label}</span></div><p className="text-sm font-medium text-navy-700">{value}</p></div>;
}
