...
export function TopologyView({ projectId, project, onOpenFile, compact = false }: Props) {
    ...
    <div className="mb-3 flex items-center justify-between gap-3">
      ...
      {stack.length > 0 && (
        <div className="flex flex-wrap justify-end gap-1.5">
          {stack.map((s) => <span key={s} className="chip bg-gray-100 text-gray-600 border border-[#d4d4d8]">{s}</span>)}
        </div>
      )}
    </div>
  );
        ...
        return (
          <foreignObject key={n.id} x={n.x} y={n.y} width={NODE_W} height={NODE_H}>
            <button
              ...
              className={`flex h-full w-full items-center gap-2 rounded-xl border px-3 text-left transition ${active ? 'border-brand-500 bg-brand-50 shadow-sm' : 'border-[#d4d4d8] bg-white hover:border-gray-300 hover:bg-gray-50'}`}
            >
  ...
  const detailBody = sel ? (
    ...
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-gray-50 border border-[#d4d4d8] text-gray-400"><Network size={22} /></div>
