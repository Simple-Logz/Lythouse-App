      ...
      {history.length===0?(
        ...
        <div className="space-y-3">
          {history.map(h=>{
            ...
            return(
              <div key={h.id} className={`card p-0 overflow-hidden border-2 ${h.status==='deployed'?'border-green-200':h.status==='rolled_back'?'border-amber-200':h.status==='failed'?'border-red-200':'border-[#18181b]'}`}>
                <div className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-gray-50" onClick={()=>setExpanded(isOpen?null:h.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {h.version&&<span className="chip bg-gray-100 text-gray-600 border border-[#18181b] text-xs">{h.version}</span>}
                  ...
                  <div className="border-t border-gray-100 bg-gray-50/50 px-4 py-4 space-y-3">
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 text-xs">
                      {[
                        ...
                        <div key={d.label} className="rounded-lg bg-white border border-[#18181b] p-2.5 text-center">
