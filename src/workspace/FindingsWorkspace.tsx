...
const BIZ={
  ...
  informational:{label:'Informational',color:'text-gray-600',bg:'bg-gray-50',border:'border-[#71717a]',dot:'bg-gray-400'},
};
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          {label:'Total Open',value:open.length,color:'text-navy-900',bg:'bg-gray-50',border:'border-[#71717a]',onClick:()=>{setSevFilter('all');setStatusFilter('open');}},
              ...
              <div className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:brightness-[0.98] ${meta.bg}`} onClick={()=>setExpanded(isOpen?null:f.id)}>
                ...
                <div className="flex items-center gap-1.5 shrink-0" onClick={e=>e.stopPropagation()}>
                  {f.status==='open'?<>
                    ...
                    <button onClick={()=>updateStatus(f.id,'ignored')} disabled={updating===f.id} className="px-2 py-1 text-xs text-gray-500 border border-[#71717a] rounded-lg hover:bg-gray-100">Ignore</button>
              ...
              {isOpen&&(
                <div className="bg-white border-t border-gray-100 px-4 py-4 space-y-4">
                  ...
                  {ai.fix&&(
                    <div className="rounded-xl border border-brand-200 bg-brand-50 px-4 py-3">
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        <button className="flex items-center gap-1.5 px-3 py-1.5 border border-[#71717a] text-gray-700 rounded-lg text-xs font-semibold hover:bg-gray-50"><GitPullRequest size={12}/>Create PR</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
                    ...
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#71717a] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100">
                    ...
                    <button onClick={onRunValidation} disabled={running} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#71717a] bg-gray-50 text-gray-700 text-xs font-medium hover:bg-gray-100">
                      <RefreshCw size={12}/>Revalidate
                    </button>
                    {f.status!=='open'&&<button onClick={()=>updateStatus(f.id,'open')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#71717a] text-gray-500 text-xs ml-auto hover:bg-gray-50">
