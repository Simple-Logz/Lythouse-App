        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 divide-y divide-gray-100">
          ...
          <div className="pb-2">
            ...
            <Row icon={<GitBranch size={14} className="text-brand-600" />} title="Branch assessed" desc="The branch LytHouse reads to build the release decision.">
              <input value={branch} onChange={(e) => setBranch(e.target.value)} className="w-32 rounded-lg border border-[#a1a1aa] px-2.5 py-1.5 text-sm focus:border-brand-400 focus:outline-none" placeholder="main" />
            </Row>
            <Row icon={<KeyRound size={14} className="text-brand-600" />} title="GitHub access token" desc="Raises the API rate limit and enables private repositories. Stored with this project only.">
              <div className="flex items-center gap-1.5">
                <input value={token} onChange={(e) => setToken(e.target.value)} type={showToken ? 'text' : 'password'} className="w-36 rounded-lg border border-[#a1a1aa] px-2.5 py-1.5 text-xs font-mono focus:border-brand-400 focus:outline-none" placeholder="ghp_…" />
          ...
          <div className="py-2">
            ...
            <Row icon={<Bell size={14} className="text-brand-600" />} title="Notify on new changes" desc={connectedProviders.length ? 'Route change-window alerts to a connected tool.' : 'Connect a tool in Integrations to enable alerts.'}>
              {connectedProviders.length ? (
                <select value={s.notifyChannel} onChange={(e) => set({ notifyChannel: e.target.value })} className="rounded-lg border border-[#a1a1aa] px-2 py-1.5 text-sm focus:border-brand-400 focus:outline-none">
          ...
          <div className="py-2">
            ...
            <Row icon={<ShieldCheck size={14} className="text-brand-600" />} title="Minimum readiness to deploy" desc="Block the Deploy action when release readiness is below this threshold.">
              <select value={s.deployGateReadiness} onChange={(e) => set({ deployGateReadiness: Number(e.target.value) })} className="rounded-lg border border-[#a1a1aa] px-2 py-1.5 text-sm focus:border-brand-400 focus:outline-none">
