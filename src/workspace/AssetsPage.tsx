...
function StatusBadge({status}:{status:string}){
  const cfg:Record<string,{color:string;bg:string;border:string;icon:any;label:string}>={
    ...
    offline:{color:'text-gray-600',bg:'bg-gray-50',border:'border-[#71717a]',icon:WifiOff,label:'Offline'},
    error:{color:'text-red-700',bg:'bg-red-50',border:'border-red-200',icon:X,label:'Error'},
    disconnected:{color:'text-gray-500',bg:'bg-gray-50',border:'border-[#71717a]',icon:WifiOff,label:'Not Connected'},
  };
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border-2 border-brand-200 bg-gradient-to-r from-brand-50 to-white px-6 py-5">
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={()=>setNotifications(n=>!n)} title={notifications?'Notifications on':'Notifications off'} className={'p-2 rounded-lg border transition-colors '+(notifications?'border-brand-300 bg-brand-50 text-brand-600':'border-[#71717a] text-gray-400')}>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {connected.length===0?(
            <div className="rounded-xl border-2 border-dashed border-[#71717a] py-10 text-center">
        ...
        <div>
          ...
          {changeEvents.length===0?(
            <div className="rounded-xl border border-[#71717a] bg-gray-50 py-8 text-center">
            ...
            <div className="space-y-2">
              {changeEvents.slice(0,8).map((evt,i)=>(
                <div key={evt.id+i} className="rounded-xl border border-[#71717a] bg-white px-3 py-3">
              ...
              <div key={cat} className="mb-6">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {catAssets.map(asset=>{
                    ...
                    return(
                      <div key={asset.id} className={'rounded-xl border-2 transition-all '+(isConnected?'border-green-200 bg-green-50/50':isConnecting?'border-brand-400 bg-white shadow-lg ring-2 ring-brand-200':'border-[#71717a] bg-white hover:border-gray-300')}>
