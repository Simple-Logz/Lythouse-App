  ...
  return (
    <div className="space-y-5">
      ...
      {stale && (() => {
        ...
        return (
          <div className={`card !p-3 border ${st.tone}`} style={{ borderLeftWidth: 4, borderLeftColor: st.bar }}>
            ...
            <div className="grid gap-2 sm:grid-cols-3 rounded-lg border border-[#d4d4d8]/70 bg-white/70 px-3 py-2">
              <div>
            ...
            <details className="group mt-3">
              ...
              <div className="mt-2 pt-2 border-t border-[#d4d4d8]/70 grid gap-4 lg:grid-cols-2">
                <div>
      ...
      <div className={`card !p-0 border ${recBg} overflow-hidden relative`} style={{ borderLeftWidth: 4, borderLeftColor: recBar }}>
        ...
        <div className="mt-2.5 pt-2.5 border-t border-[#d4d4d8]/70">
      ...
      {(() => {
        ...
        return (
          <div className="card border-brand-200 bg-brand-50/40">
            ...
            {guided.length > 0 && (
              <div className="mt-4 pt-3 border-t border-[#d4d4d8]/60">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {[`${r.appType}`, `${r.services} services`, ...r.cloud, r.orchestration, r.ci, r.infra, ...r.monitoring].filter((x) => x && x !== '—').map((x, i) => (
            <span key={i} className="inline-flex items-center rounded-lg border border-[#d4d4d8] bg-white px-2.5 py-1.5 text-xs font-medium text-navy-700">{x}</span>
          ))}
        </div>
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 space-y-4">
          <div className="grid gap-2.5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
            {[{ k: 'Application', v: r.appType }, { k: 'Services', v: String(r.services || '—') }, { k: 'Cloud', v: r.cloud.join(', ') || '—' }, { k: 'Containers', v: r.container }, { k: 'Orchestration', v: r.orchestration }, { k: 'Infrastructure', v: r.infra }, { k: 'CI/CD', v: r.ci }, { k: 'Monitoring', v: r.monitoring.join(', ') || '—' }, { k: 'Environments', v: r.envs.length ? String(r.envs.length) : '—' }].map((g) => (
              <div key={g.k} className="rounded-xl border border-[#d4d4d8] px-3 py-2.5"><div className="text-[10px] uppercase tracking-wide text-gray-400">{g.k}</div><div className="text-sm font-semibold text-navy-900 mt-0.5 leading-tight">{g.v}</div></div>
            ))}
          </div>
...
function ArchNode({ icon, label, accent }) {
  return <span className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium ${accent ? 'border-brand-300 bg-brand-50 text-brand-800' : 'border-[#d4d4d8] bg-white text-navy-700'}`}><span className="text-gray-400">{icon}</span>{label}</span>;
}
