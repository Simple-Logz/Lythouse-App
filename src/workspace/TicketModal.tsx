      ...
      <div className="w-full max-w-lg animate-scale-in rounded-xl bg-white p-5 shadow-xl overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {providers.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => { setProvider(p.id); setResult(null); }} className={`relative flex flex-col items-center gap-1.5 rounded-xl border-2 px-2 py-2.5 transition-all ${active ? 'border-brand-500 bg-brand-50' : 'border-[#18181b] hover:border-gray-300'}`}>
