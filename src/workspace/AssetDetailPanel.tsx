...
function StateBadge({state}:{state:string}){
  ...
  const cls=ok.includes(s)?'bg-green-50 text-green-700 border-green-200':
    ...
    'bg-gray-50 text-gray-600 border-[#18181b]';
    ...
    <div>
      ...
      <div className="space-y-2">
        {data.sites?.map((site:any)=>(
          <div key={site.id} className="rounded-xl border border-[#18181b] bg-white px-4 py-3">
    ...
    {data.recentRuns?.length>0&&<div>
      ...
      <div className="space-y-1.5">
        {data.recentRuns.map((r:any,i:number)=>(
          <div key={i} className="flex items-center gap-3 rounded-lg border border-[#18181b] px-3 py-2">
    ...
    {data.recentCommits?.length>0&&<div>
      ...
      <div className="space-y-1.5">
        {data.recentCommits.map((c:any,i:number)=>(
          <div key={i} className="flex items-start gap-3 rounded-lg border border-[#18181b] px-3 py-2">
    ...
    {data.openPRs?.length>0&&<div>
      ...
      <div className="space-y-1.5">
        {data.openPRs.map((pr:any)=>(
          <div key={pr.number} className="flex items-center gap-2 rounded-lg border border-[#18181b] px-3 py-2">
            ...
            {pr.draft&&<span className="text-[10px] text-gray-400 border border-[#18181b] px-1.5 py-0.5 rounded">Draft</span>}
    ...
    {data.recentDeploys?.length>0&&<div>
      ...
      <div className="space-y-2">
        {data.recentDeploys.map((d:any,i:number)=>(
          <div key={i} className="rounded-xl border border-[#18181b] px-4 py-3">
...
function GenericData({data,source}:{data:any;source:string}){
  return<div className="space-y-4">
    <div className="rounded-xl border border-[#18181b] bg-gray-50 px-4 py-3">
          ...
          {view==='details'&&(
            <div className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {assetMeta.watches.map(w=>(
                    <span key={w} className="flex items-center gap-1.5 text-xs bg-white border border-[#18181b] text-gray-600 px-3 py-1.5 rounded-full">
              ...
              <div>
                ...
                <div className="rounded-xl border border-[#18181b] bg-gray-50 px-4 py-3 space-y-2">
                  ...
                  <p className="text-[10px] text-gray-400 pt-1 border-t border-[#18181b]">Secrets are never shown in full and are encrypted at rest.</p>
                </div>
              </div>
              <div className="rounded-xl border border-[#18181b] bg-white px-4 py-3 space-y-1 text-xs">
