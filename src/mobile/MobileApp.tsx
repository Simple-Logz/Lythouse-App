...
export const TONE = {
  ...
  gray: { text: 'text-gray-500', bg: 'bg-gray-100', border: 'border-[#71717a]', dot: 'bg-gray-400' },
};
        ...
        {blockers.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {blockers.map((f) => (
                <div key={f.id} className="rounded-xl border border-[#71717a] bg-white p-3">
...
export function ApprovalCard({ release, onApprove }) {
  ...
  return (
    <div className={`rounded-2xl border p-3 ${allApproved ? 'border-[#9adcb4] bg-[#e3f7ea]' : 'border-[#71717a] bg-white'}`}>
...
export function AccountScreen({ user, profile, signOut }) {
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="rounded-2xl border border-[#71717a] bg-white p-4 flex items-center gap-3">
      ...
      <div className="rounded-2xl border border-[#71717a] bg-white p-4 flex items-start gap-3">
      ...
      <button onClick={() => signOut()} className="w-full rounded-xl border border-[#71717a] bg-white py-3 text-sm font-semibold text-[#dc2626] flex items-center justify-center gap-2 active:bg-gray-50"><LogOut size={15} />Sign out</button>
    </div>
  );
}
