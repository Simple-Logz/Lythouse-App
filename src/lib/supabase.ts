...
export type Project={id:string;workspace_id:string;stack_id?:string|null;name:string;description:string|null;git_url:string;git_branch:string;repo_folder:string;language:string|null;framework:string|null;status:string;created_by:string|null;created_at:string};
...
export type Deployment={id:string;project_id:string;workspace_id:string;validation_id:string|null;environment_id:string|null;change_request_id:string|null;status:string;provider:string|null;external_id:string|null;deployed_by:string|null;started_at:string|null;created_at:string;completed_at:string|null};
...
export async function resolveActiveWorkspace():Promise<Workspace>{
 const{data:{user},error:authError}=await supabase.auth.getUser();
 if(authError||!user)throw new Error('Your session expired. Sign in again.');
 let{data:members,error:memberError}=await supabase.from('workspace_members').select('workspace_id,created_at').eq('user_id',user.id).order('created_at',{ascending:true});
 if(memberError)throw memberError;
 if(!members?.length){const{data,error}=await supabase.rpc('bootstrap_user_workspace',{p_name:'My Workspace'});if(error)throw error;members=data?.workspace_id?[{workspace_id:data.workspace_id,created_at:new Date().toISOString()}]:[];}
 const ids=(members||[]).map((m:any)=>m.workspace_id);
 if(!ids.length)throw new Error('No accessible workspace was found.');
 const{data:workspaces,error}=await supabase.from('workspaces').select('id,name,slug,owner_id,description,organization_id,created_at').in('id',ids);
 if(error)throw error;
 if(!workspaces?.length)throw new Error('No accessible workspace was found.');
 const saved=localStorage.getItem('sandbox.activeWs');
 const selected=workspaces.find((w:any)=>w.id===saved)||workspaces[0];
 localStorage.setItem('sandbox.activeWs',selected.id);
 return selected as Workspace;
}
...
export type DependencyEdge={id:string;environment_id:string;workspace_id:string;source_component_id:string;target_component_id:string;relationship_type:string;evidence:Record<string,unknown>;human_confirmed:boolean;created_at:string;resolved_at:string|null};
...
export const CONNECTION_METHODS=[{id:'agent'as ConnectionMethod,label:'Per-Server Agent',description:'Install a lightweight agent on each server. Outbound HTTPS only.',plans:['developer'as PlanId,'enterprise'as PlanId]},{id:'ssh'as ConnectionMethod,label:'Agentless SSH',description:'Connect to Linux/Unix servers via SSH with a read-only account.',plans:['developer'as PlanId,'enterprise'as PlanId]},{id:'winrm'as ConnectionMethod,label:'Agentless WinRM',description:'Connect to Windows servers via PowerShell Remoting over HTTPS.',plans:['developer'as PlanId,'enterprise'as PlanId]},{id:'collector'as ConnectionMethod,label:'Enterprise Collector',description:'Deploy a collector inside your network. For regulated environments.',plans:['enterprise'as PlanId]},{id:'offline'as ConnectionMethod,label:'Offline Snapshot Import',description:'Air-gapped environments. Collect, redact, and upload a signed snapshot bundle.',plans:['free'as PlanId,'developer'as PlanId,'enterprise'as PlanId]}]as const;
...
export const PLANS:PLANS={free:{id:'free',name:'Free',price:0,color:'bg-gray-100 text-gray-700',badge:'bg-gray-100 text-gray-600 border-[#d4d4d8]'},developer:{id:'developer',name:'Developer',price:29,color:'bg-brand-50 text-brand-700',badge:'bg-brand-50 text-brand-700 border-brand-200'},enterprise:{id:'enterprise',name:'Enterprise',price:199,color:'bg-navy-800 text-white',badge:'bg-navy-800 text-white border-navy-700'}};
