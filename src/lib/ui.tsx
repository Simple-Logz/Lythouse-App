...
const m:Record<string,{label:string;cls:string}>={pending:{label:'Pending',cls:'bg-gray-100 text-gray-600 border border-[#a1a1aa]'},running:{label:'Running',cls:'bg-brand-50 text-brand-700 border border-brand-200'},completed:{label:'Completed',cls:'bg-brand-50 text-brand-700 border border-brand-200'},failed:{label:'Failed',cls:'bg-red-50 text-danger-600 border border-red-200'},skipped:{label:'Skipped',cls:'bg-gray-100 text-gray-500 border border-[#a1a1aa]'}};
...
const m:Record<string,{label:string;cls:string;icon:ReactNode}>={critical:{label:'Critical',cls:'bg-red-50 text-danger-600 border border-red-200',icon:<ShieldX size={11}/>},high:{label:'High',cls:'bg-amber-50 text-amber-600 border border-amber-200',icon:<ShieldAlert size={11}/>},medium:{label:'Medium',cls:'bg-blue-50 text-blue-600 border border-blue-200',icon:<AlertTriangle size={11}/>},low:{label:'Low',cls:'bg-gray-100 text-gray-600 border border-[#a1a1aa]',icon:<Check size={11}/>},none:{label:'Clean',cls:'bg-brand-50 text-brand-700 border border-brand-200',icon:<ShieldCheck size={11}/>}};
const s=m[severity]??m.low;return<span className={'chip '+s.cls}>{s.icon}{s.label}</span>;
}
export function FindingStatusBadge({status}:{status:FindingStatus}){
const m:Record<string,{label:string;cls:string}>={open:{label:'Open',cls:'bg-red-50 text-danger-600 border border-red-200'},resolved:{label:'Resolved',cls:'bg-brand-50 text-brand-700 border border-brand-200'},ignored:{label:'Ignored',cls:'bg-gray-100 text-gray-500 border border-[#a1a1aa]'}};
...
if(score===null)return<div className="flex shrink-0 items-center justify-center rounded-full border-2 border-dashed border-[#a1a1aa] text-gray-300" style={{width:size,height:size}} title="No score — validation didn't complete"><span className="font-bold leading-none" style={{fontSize:Math.max(12,size*0.3)}}>–</span></div>;
...
return<div className={b+' border-[#a1a1aa] bg-white text-gray-400'}>{icon}</div>;
}
export function EmptyState({icon,title,description,action}:{icon:ReactNode;title:string;description:string;action?:ReactNode}){
return<div className="flex flex-col items-center justify-center px-6 py-20 text-center">
<div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gray-50 border border-[#a1a1aa] text-gray-400">{icon}</div>
