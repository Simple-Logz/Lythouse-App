...
function PipelineDiagram() {
  ...
  return (
    <div className="my-5 rounded-2xl border border-[#d4d4d8] bg-gradient-to-b from-[#faf9ff] to-white p-4">
...
export function DocsPage() {
    ...
    <nav className="px-3 py-4">
      <div className="relative mb-4">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter docs…"
          className="w-full rounded-lg border border-[#d4d4d8] bg-gray-50 py-2 pl-8 pr-3 text-[13px] text-navy-900 placeholder:text-gray-400 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
      </div>
      {filtered.map((g) => (
  ...
  return (
    <div className={dark ? 'dark min-h-screen text-[#eeecf6]' : 'min-h-screen bg-white text-navy-900'} style={dark ? { background: 'linear-gradient(180deg,#211c30 0%,#171324 55%)' } : undefined}>
      {/* Top bar */}
      <header className="fixed inset-x-0 top-0 z-40 h-14 border-b border-[#d4d4d8] bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-full max-w-[1400px] items-center gap-3 px-4">
          <button onClick={() => setNavOpen((v) => !v)} className="lg:hidden rounded-lg border border-[#d4d4d8] p-2 text-gray-600"><Menu size={16} /></button>
          ...
          <button onClick={toggleTheme} title="Toggle light / dark" className="grid h-9 w-9 place-items-center rounded-lg border border-[#d4d4d8] text-navy-600 hover:bg-gray-50">{dark ? <Sun size={16} /> : <Moon size={16} />}</button>
          {session ? (
      ...
      {navOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 overflow-y-auto bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-[#d4d4d8] px-4 py-3"><span className="font-bold">Documentation</span><button onClick={() => setNavOpen(false)}><X size={18} /></button></div>
            {NavTree}
          </div>
        </div>
      )}
      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 lg:grid-cols-[256px_minmax(0,1fr)] xl:grid-cols-[256px_minmax(0,1fr)_248px]">
        {/* Left nav */}
        <aside className="hidden lg:block border-r border-[#d4d4d8]">
          ...
          <div ref={contentRef} className="mx-auto max-w-3xl">
            ...
            <section id="concepts" className="scroll-mt-24 mb-12">
              ...
              {[
                ...
                <div key={term} className="rounded-xl border border-[#d4d4d8] bg-white p-4 mb-2.5">
  ...
  --provider gcp --token <token> --endpoint <url>`}</CodeBlock>
              ...
              <div className="overflow-x-auto no-scrollbar my-3">
                <table className="w-full text-[13px] border border-[#d4d4d8] rounded-xl overflow-hidden">
            ...
            <section id="faq" className="scroll-mt-24 mb-8">
              ...
              {[
                ...
                <div key={q2} className="rounded-xl border border-[#d4d4d8] bg-white p-4 mb-2.5">
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto px-4 py-6">
            ...
            <nav className="space-y-0.5 border-l border-[#d4d4d8]">
              {FLAT.map((s) => (
