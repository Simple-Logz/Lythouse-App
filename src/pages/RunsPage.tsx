...
const CSS = `
.runs{max-width:1180px;margin:0 auto;display:flex;flex-direction:column;gap:16px}.runs-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;background:var(--lh-surface);border:1px solid var(--lh-border);border-radius:14px;padding:12px}.runs-toolbar select,.runs-toolbar button{font:inherit;font-size:13px;border:1px solid var(--lh-border);border-radius:9px;background:var(--lh-surface);color:var(--lh-text);padding:8px 10px}.runs-toolbar button{display:flex;align-items:center;gap:6px;cursor:pointer;font-weight:700}.runs-spacer{flex:1}.runs-count{font-size:12px;color:var(--lh-text2)}.runs-error{border:1px solid #f1a7a7;background:#fff3f3;color:#b42318;border-radius:12px;padding:12px 14px;font-size:13px;font-weight:600}.runs-card{background:var(--lh-surface);border:1px solid var(--lh-border);border-radius:14px;overflow:hidden}.runs-row{display:grid;grid-template-columns:1.6fr .8fr .8fr .8fr .8fr 1fr;gap:12px;align-items:center;padding:13px 16px;border-top:1px solid var(--lh-border);text-decoration:none;color:var(--lh-text)}.runs-row:first-child{border-top:0}.runs-row.h{font-size:10px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--lh-text3);background:var(--lh-surface2)}.runs-row:not(.h):hover{background:var(--lh-surface2)}.runs-project{font-weight:700}.runs-sub{font-size:11px;color:var(--lh-text3);margin-top:2px}.runs-status{font-size:12px;font-weight:700;text-transform:capitalize}.runs-empty{text-align:center;padding:60px 20px;color:var(--lh-text3)}.runs-empty svg{margin:0 auto 12px}.runs-empty h3{color:var(--lh-text);margin:0 0 5px;font-size:15px}@media(max-width:850px){.runs-row{grid-template-columns:1.5fr 1fr 1fr}.runs-row>*:nth-child(4),.runs-row>*:nth-child(5),.runs-row>*:nth-child(6){display:none}}
`
...
export function RunsPage(){
  ...
  return <div className="runs"><style>{CSS}</style>
    ...
    <div className="runs-toolbar">
      ...
      <button type="button" onClick={()=>setReload(x=>x+1)}><RefreshCw size={14}/>Refresh</button>
    </div>
    <div className="runs-card">
      ...
      {filtered.length?filtered.map(r=><Link key={r.id} to={`/projects/${r.project_id}?tab=validations&validation=${r.id}`} className="runs-row">
        ...
        <span className="runs-status">{r.status}</span><span>{r.risk_score==null?'—':r.risk_score}</span><span>{r.total_findings??0}</span><span>{r.duration_ms?fmtDuration(r.duration_ms):'—'}</span><span>{new Date(r.created_at).toLocaleString()}</span>
      </Link>):<div className="runs-empty"><ShieldCheck size={30}/><h3>No validation runs yet</h3><div>Run a validation on a connected project. Each run will be stored separately and appear here.</div></div>}
    </div>
  </div>
}
