...
const CSS = `
.runs{max-width:1180px;margin:0 auto;display:flex;flex-direction:column;gap:14px}.runs-toolbar{display:flex;gap:9px;align-items:center;flex-wrap:wrap;background:var(--lh-surface);border:1px solid var(--lh-border);border-radius:14px;padding:10px 12px}.runs-toolbar select,.runs-toolbar button{font:inherit;font-size:12px;border:1px solid var(--lh-border);border-radius:9px;background:var(--lh-surface);color:var(--lh-text);padding:7px 10px}.runs-toolbar button{display:flex;align-items:center;gap:6px;cursor:pointer;font-weight:700}.runs-spacer{flex:1}.runs-count{font-size:11px;color:var(--lh-text2)}.runs-error{border:1px solid #f1a7a7;background:#fff3f3;color:#b42318;border-radius:12px;padding:12px 14px;font-size:13px;font-weight:600}.runs-card{background:var(--lh-surface);border:1px solid var(--lh-border);border-radius:14px;overflow:hidden}.runs-row{display:grid;grid-template-columns:1.45fr .7fr .55fr .62fr .65fr 1.05fr 20px;gap:10px;align-items:center;min-height:52px;padding:7px 16px;border-top:1px solid var(--lh-border);text-decoration:none;color:var(--lh-text);font-size:12px}.runs-row:first-child{border-top:0}.runs-row.h{min-height:34px;padding-top:7px;padding-bottom:7px;font-size:9px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--lh-text3);background:var(--lh-surface2)}.runs-row:not(.h):hover{background:var(--lh-surface2)}.runs-project{font-size:13px;line-height:1.2;font-weight:700}.runs-sub{font-size:10px;line-height:1.2;color:var(--lh-text3);margin-top:2px}.runs-status{display:inline-flex;width:max-content;align-items:center;gap:5px;border-radius:999px;padding:4px 8px;font-size:10px;line-height:1;font-weight:700;text-transform:capitalize;border:1px solid var(--lh-border);background:var(--lh-surface2)}.runs-status:before{content:'';width:5px;height:5px;border-radius:50%;background:#94a3b8}.runs-status.running{color:#067647;background:#ecfdf3;border-color:#d1fadf}.runs-status.running:before{background:#12b76a}.runs-status.completed{color:#067647;background:#ecfdf3;border-color:#d1fadf}.runs-status.completed:before{background:#12b76a}.runs-status.failed{color:#b42318;background:#fef3f2;border-color:#fee4e2}.runs-status.failed:before{background:#f04438}.runs-status.pending{color:#b54708;background:#fffaeb;border-color:#fef0c7}.runs-status.pending:before{background:#f79009}.runs-chevron{color:var(--lh-text3);justify-self:end}.runs-empty{text-align:center;padding:42px 20px;color:var(--lh-text3);font-size:12px}.runs-empty svg{margin:0 auto 10px}.runs-empty h3{color:var(--lh-text);margin:0 0 5px;font-size:14px}@media(max-width:850px){.runs-row{grid-template-columns:1.5fr 1fr .9fr 18px;min-height:50px;padding:7px 12px}.runs-row>*:nth-child(3),.runs-row>*:nth-child(5),.runs-row>*:nth-child(6){display:none}.runs-row.h>*:nth-child(4){display:block}}
`
...
export function RunsPage(){
  ...
  return <div className="runs"><style>{CSS}</style>
    ...
    <div className="runs-card">
      <div className="runs-row h"><span>Project</span><span>Status</span><span>Risk</span><span>Findings</span><span>Duration</span><span>Started</span><span/></div>
      {filtered.length?filtered.map(r=><Link key={r.id} to={`/projects/${r.project_id}?tab=validations&validation=${r.id}`} className="runs-row">
        ...
        <span><span className={`runs-status ${String(r.status||'').toLowerCase()}`}>{r.status}</span></span><span>{r.risk_score==null?'—':r.risk_score}</span><span>{r.total_findings??0}</span><span>{r.duration_ms?fmtDuration(r.duration_ms):'—'}</span><span>{new Date(r.created_at).toLocaleString()}</span><ChevronRight size={14} className="runs-chevron"/>
