...
function CustomTooltip({active,payload,label}:any){
  ...
  if(!d)return null;
  return(
    <div className="bg-white border border-[#18181b] rounded-xl shadow-xl p-4 min-w-52">
  ...
  return(
    <div>
      ...
      {chartData.every(d=>d.total===0)?(
        <div className="flex items-center justify-center h-48 text-sm text-gray-400 bg-gray-50 rounded-xl border border-[#18181b]">
        ...
        <ResponsiveContainer width="100%" height={260}>
          {chartType==='bar'?(
            <BarChart data={chartData} margin={{top:5,right:30,left:0,bottom:5}} barCategoryGap="30%" onClick={d=>d?.activePayload&&setSelectedPoint(d.activePayload[0]?.payload)}>
              ...
              <Bar dataKey="passed" name="Passed" fill="#4f45ab" stackId="a" radius={[0,0,0,0]}/>
              <Bar dataKey="blocked" name="Blocked" fill="#6252b3" stackId="a"/>
              <Bar dataKey="failed" name="Failed" fill="#322c66" stackId="a"/>
            ...
            <LineChart data={chartData} margin={{top:5,right:30,left:0,bottom:5}} onClick={d=>d?.activePayload&&setSelectedPoint(d.activePayload[0]?.payload)}>
              ...
              <Tooltip content={<CustomTooltip/>}/>
              <Legend iconSize={10} iconType="circle" wrapperStyle={{fontSize:11}}/>
              <Line dataKey="passed" name="Passed" stroke="#4f45ab" strokeWidth={2} dot={{r:3}} activeDot={{r:5}}/>
              <Line dataKey="blocked" name="Blocked" stroke="#6252b3" strokeWidth={2} dot={{r:3}}/>
              <Line dataKey="failed" name="Failed" stroke="#322c66" strokeWidth={2} dot={{r:3}}/>
      ...
      {/* Drill-down panel */}
      {selectedPoint&&(
        <div className="mt-4 rounded-xl border border-[#18181b] bg-gray-50 p-4">
          ...
          {selectedPoint.releases.length===0?(
            ...
            <div className="space-y-1.5">
              {selectedPoint.releases.map((r,i)=>(
                <div key={i} className="flex items-center gap-3 rounded-lg bg-white border border-[#18181b] px-3 py-2.5 text-xs">
