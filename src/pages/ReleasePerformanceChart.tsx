...
function CustomTooltip({active,payload,label}:any){
  ...
  if(!d)return null;
  return(
    <div className="bg-white border border-[#d4d4d8] rounded-xl shadow-xl p-4 min-w-52">
  ...
  return(
    <div>
      ...
      {chartData.every(d=>d.total===0)?(
        <div className="flex items-center justify-center h-48 text-sm text-gray-400 bg-gray-50 rounded-xl border border-[#d4d4d8]">
      ...
      {/* Drill-down panel */}
      {selectedPoint&&(
        <div className="mt-4 rounded-xl border border-[#d4d4d8] bg-gray-50 p-4">
          ...
          {selectedPoint.releases.length===0?(
            ...
            <div className="space-y-1.5">
              {selectedPoint.releases.map((r,i)=>(
                <div key={i} className="flex items-center gap-3 rounded-lg bg-white border border-[#d4d4d8] px-3 py-2.5 text-xs">
