...
function Tip({text}:{text:string}){
  ...
  return<span className="relative inline-flex ml-1 align-middle group">
    ...
    {show&&<span className="absolute z-50 w-60 rounded-lg border border-[#a1a1aa] bg-white px-3 py-2 text-xs text-gray-600 shadow-xl leading-relaxed" style={{top:'1.6rem',left:'50%',transform:'translateX(-50%)',whiteSpace:'normal'}}>
      <span className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-white border-l border-t border-[#a1a1aa] rotate-45 block"/>
      {text}
    </span>}
  </span>;
}
...
export function LoadTestingPage(){
  ...
  const statusColor=(s:TestStatus)=>s==='completed'?'bg-green-50 text-green-700 border-green-200':s==='running'?'bg-blue-50 text-blue-700 border-blue-200':s==='failed'?'bg-red-50 text-danger-600 border-red-200':'bg-gray-50 text-gray-600 border-[#a1a1aa]';
          ...
          <div className="border-t border-gray-100 bg-gray-50/50 px-5 py-4">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-4">
              {[['P95 Latency',`${run.p95}ms`,run.p95!>run.latencySla],['P99 Latency',`${run.p99}ms`,run.p99!>run.latencySla],['Actual RPS',run.actualRps?.toLocaleString()+'',false],['Error Rate',`${run.errorRate}%`,(run.errorRate??0)>1]].map(([l,v,warn]:any)=>(
                <div key={l} className={`rounded-lg border p-3 ${warn?'border-red-200 bg-red-50':'border-[#a1a1aa] bg-white'}`}>
