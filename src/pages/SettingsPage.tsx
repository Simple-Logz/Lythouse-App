...
export function SettingsPage(){
  ...
  return<div>
    ...
    {/* Tabs */}
    <div className="mb-6 flex gap-1 border-b border-[#d4d4d8] overflow-x-auto no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
      {TABS.map(t=>(
        ...
        <div className="card">
          ...
          <div className="flex items-center gap-5 mb-5 pb-5 border-b border-gray-100">
            <div className="relative group shrink-0">
              {avatarUrl?(
                <img src={avatarUrl} alt="Avatar" className="h-20 w-20 rounded-full object-cover border-2 border-[#d4d4d8]"/>
              ):(
                <div className="flex h-20 w-20 items-center justify-center rounded-full bg-brand-100 text-brand-700 text-3xl font-bold border-2 border-[#d4d4d8]">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[{id:'light',label:'Light',icon:Sun},{id:'dark',label:'Dark',icon:Moon},{id:'system',label:'System',icon:Monitor}].map(t=>(
              <button key={t.id} onClick={()=>setTheme(t.id as any)} className={`flex flex-col items-center gap-2 rounded-xl border-2 p-4 transition-all ${appPref.theme===t.id?'border-brand-500 bg-brand-50':'border-[#d4d4d8] hover:border-gray-300'}`}>
