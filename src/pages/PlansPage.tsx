...
{!canManage&&wsId()&&(
  <div className="mb-6 flex items-start gap-3 rounded-xl border border-[#71717a] bg-gray-50 px-4 py-3">
...
{ORDER.map(id=>{
  ...
  return(
    <div key={id}
      ...
      className={`relative flex flex-col rounded-2xl border p-6 transition-all duration-200 ${dark?'text-white -translate-y-1':`bg-white text-navy-900 ${isCurrent?'border-brand-400 ring-1 ring-brand-300':'border-[#71717a]'}`}`}
      ...
      {isCurrent?(
        ...
        hasSubscription&&canManage?(
          ...
          <button disabled className={`mt-6 w-full cursor-default rounded-xl border px-4 py-2.5 text-sm font-semibold ${dark?'border-white/20 text-white/70':'border-[#71717a] text-gray-400'}`}>Free forever</button>
        )
      ):(
