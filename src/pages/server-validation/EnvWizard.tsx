              ...
              {STEPS.map((s, i) => {
                ...
                return (
                  <button key={i} onClick={() => i <= step && setStep(i)} disabled={i > step}
                    ...
                    <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 ${active ? 'border-brand-500 bg-brand-500 text-white' : done ? 'border-brand-300 bg-brand-50 text-brand-600' : 'border-[#d4d4d8] bg-white'}`}>
