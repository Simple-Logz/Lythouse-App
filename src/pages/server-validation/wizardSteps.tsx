      ...
      {CONNECTION_METHODS.map(m => {
        ...
        return (
          <button key={m.id} onClick={() => allowed && setForm({ ...form, method: m.id })}
            className={`flex w-full items-start gap-3 rounded-xl border p-4 text-left transition-all ${selected ? 'border-brand-500 bg-brand-50' : allowed ? 'border-[#d4d4d8] hover:border-gray-300' : 'border-gray-100 bg-gray-50 opacity-60 cursor-not-allowed'}`}>
...
export function StepCredentials({ form, setForm }: { form: any; setForm: (f: any) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border-2 border-dashed border-[#d4d4d8] p-8 text-center">
...
export function StepTargets({ targets, setTargets }: { targets: Partial<ServerTarget>[]; setTargets: (t: Partial<ServerTarget>[]) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {targets.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[#d4d4d8] py-10 text-center">
    ...
    <div>
      ...
      <div className="space-y-1.5">
        {items.map(c => {
          ...
          return (
            <button key={c} onClick={() => toggle(c, field)} className={`flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-left text-sm transition-all ${checked ? 'border-brand-300 bg-brand-50' : 'border-gray-100 hover:border-[#d4d4d8]'}`}>
...
export function StepPreCheck({ targets, results, checking }: { targets: Partial<ServerTarget>[]; results: Record<number, CheckResult>; checking: boolean }) {
  const checkIcon = (status: string) => {
    ...
    return <div className="h-3 w-3 rounded-full border border-[#d4d4d8]" />;
  };
            ...
            return (
              <div key={i} className="grid grid-cols-7 gap-2 border-b border-gray-50 px-4 py-3 items-center">
                ...
                {(['dns', 'network', 'auth', 'os', 'permissions', 'agent'] as const).map(k => (
                  <div key={k} className="col-span-1 flex justify-center">{r ? checkIcon(r[k]) : <div className="h-3 w-3 rounded-full border border-[#d4d4d8]" />}</div>
                ))}
              </div>
            );
          })}
        ...
        <>
          <div className="card">
            ...
            <div className="flex flex-wrap gap-1.5">
              {components.map(c => (
                <span key={c.id} className="chip bg-gray-50 text-gray-600 border border-[#d4d4d8]">
