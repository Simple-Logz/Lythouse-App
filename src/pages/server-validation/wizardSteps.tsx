      ...
      {CONNECTION_METHODS.map(m => {
        ...
        return (
          <button key={m.id} onClick={() => allowed && setForm({ ...form, method: m.id })}
            className={`flex w-full items-start gap-3 rounded-xl border p-4 text-left transition-all ${selected ? 'border-brand-500 bg-brand-50' : allowed ? 'border-gray-200 hover:border-gray-300' : 'border-gray-100 bg-gray-50 opacity-60 cursor-not-allowed'}`}>
    ...
    <div>
      ...
      <div className="space-y-1.5">
        {items.map(c => {
          ...
          return (
            <button key={c} onClick={() => toggle(c, field)} className={`flex w-full items-center gap-2 rounded-lg border px-3 py-2 text-left text-sm transition-all ${checked ? 'border-brand-300 bg-brand-50' : 'border-gray-100 hover:border-gray-200'}`}>
...
export function StepPreCheck({ targets, results, checking }: { targets: Partial<ServerTarget>[]; results: Record<number, CheckResult>; checking: boolean }) {
  const checkIcon = (status: string) => {
    ...
    return <div className="h-3 w-3 rounded-full border border-gray-200" />;
  };
            ...
            return (
              <div key={i} className="grid grid-cols-7 gap-2 border-b border-gray-50 px-4 py-3 items-center">
                ...
                {(['dns', 'network', 'auth', 'os', 'permissions', 'agent'] as const).map(k => (
                  <div key={k} className="col-span-1 flex justify-center">{r ? checkIcon(r[k]) : <div className="h-3 w-3 rounded-full border border-gray-200" />}</div>
                ))}
              </div>
            );
          })}
        ...
        <>
          <div className="card">
            ...
            <div className="flex flex-wrap gap-1.5">
              {components.map(c => (
                <span key={c.id} className="chip bg-gray-50 text-gray-600 border border-gray-200">
