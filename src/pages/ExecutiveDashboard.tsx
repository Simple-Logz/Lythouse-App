      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={()=>setShowFilters(f=>!f)} className={'flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors '+(showFilters||riskFilter.size>0?'border-brand-500 bg-brand-50 text-brand-700':'border-[#18181b] text-gray-600 hover:bg-gray-50')}>
        ...
        {showFilters&&(
          <div className="mb-4 p-4 rounded-xl border border-[#18181b] bg-gray-50 space-y-3">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[{id:'critical',label:'Critical',color:'bg-red-100 text-red-700 border-red-300'},{id:'high',label:'High',color:'bg-orange-100 text-orange-700 border-orange-300'},{id:'medium',label:'Medium',color:'bg-amber-100 text-amber-700 border-amber-300'},{id:'low',label:'Low',color:'bg-green-100 text-green-700 border-green-300'}].map(r=>(
                  <button key={r.id} onClick={()=>{const n=new Set(riskFilter);n.has(r.id)?n.delete(r.id):n.add(r.id);setRiskFilter(n);}} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(riskFilter.has(r.id)?r.color+' ring-2 ring-offset-1 ring-current':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[{id:'has-critical',label:'Has Critical Issues'},{id:'has-high',label:'Has High Issues'},{id:'no-issues',label:'Clean (No Issues)'}].map(r=>(
                  <button key={r.id} onClick={()=>{const n=new Set(riskFilter);n.has(r.id)?n.delete(r.id):n.add(r.id);setRiskFilter(n);}} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(riskFilter.has(r.id)?'border-brand-500 bg-brand-50 text-brand-700 ring-2 ring-offset-1 ring-brand-400':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>
                    {riskFilter.has(r.id)?'✓ ':''}{r.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={()=>setWsFilter('all')} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(wsFilter==='all'?'border-brand-500 bg-brand-50 text-brand-700':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>All Workspaces</button>
                {workspaces.map(w=>(
                  <button key={w.id} onClick={()=>setWsFilter(w.id)} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(wsFilter===w.id?'border-brand-500 bg-brand-50 text-brand-700 ring-2 ring-offset-1 ring-brand-400':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={()=>setOwnerFilter('all')} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(ownerFilter==='all'?'border-brand-500 bg-brand-50 text-brand-700':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>All Owners</button>
                {creators.map(c=>(
                  <button key={c.id} onClick={()=>setOwnerFilter(c.id)} className={'px-3 py-1.5 rounded-lg border text-xs font-medium transition-all '+(ownerFilter===c.id?'border-brand-500 bg-brand-50 text-brand-700 ring-2 ring-offset-1 ring-brand-400':'border-[#18181b] bg-white text-gray-600 hover:bg-gray-100')}>
