...
const verdictStyle=verdict==='go'?'border-green-200 bg-green-50 text-green-700':verdict==='conditional'?'border-amber-200 bg-amber-50 text-amber-700':verdict==='no-go'?'border-red-200 bg-red-50 text-danger-600':'border-[#d4d4d8] bg-gray-50 text-gray-600';
...
<span className={`chip border text-xs ${ENV_COLORS[s.environment]??'bg-gray-100 text-gray-600 border-[#d4d4d8]'}`}>{s.environment}</span>
...
<div key={label as string} className="rounded-lg border border-[#d4d4d8] bg-white px-3 py-2.5 text-center">
...
<span key={svc} className="chip bg-gray-100 text-gray-700 border border-[#d4d4d8]">{svc}</span>
))}
</div>
</div>
)}
