...
const load=async()=>{
  ...
  if(wsList.length){
    ...
    (planData??[]).forEach(p=>{if(!map[p.workspace_id])map[p.workspace_id]=p;});
    setPlans(map);
  }
  setLoading(false);
};
...
const setActive=(w:Workspace)=>{
  ...
  // Reload to refresh the AppShell context
...
{workspaces.length===0
?<EmptyState icon={<Boxes size={22}/>} title="No workspaces yet" description="Create your first workspace to start organizing projects and validations." action={<button onClick={()=>setCreating(true)} className="btn-primary"><Plus size={16}/> New workspace</button>}/>
  ...
  return(
    <div key={w.id} className={`card group text-left transition-all hover:shadow-md hover:-translate-y-0.5 ${isActive?'ring-2 ring-brand-500':''}`}>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <button onClick={()=>{
  localStorage.setItem('sandbox.activeWs', w.id);
  setWorkspaces(prev=>prev.map(ws=>ws.id===w.id?ws:ws));
  navigate('/projects');
