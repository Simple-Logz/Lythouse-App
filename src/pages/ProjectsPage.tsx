        ...
        <Link key={p.id} to={`/projects/${p.id}`} className="group flex items-center gap-4 px-4 py-3.5 transition-colors hover:bg-gray-50/70">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              {p.language&&<span className="chip border border-[#18181b] bg-gray-100 text-[11px] text-gray-600">{p.language}</span>}
            </div>
...
<div className="mb-4 flex rounded-lg border border-[#18181b] p-0.5 text-xs font-semibold">
      ...
      <>
        ...
        <div className="space-y-1.5 max-h-[46vh] overflow-y-auto -mx-1 px-1">
          {repos.filter(r=>r.full_name?.toLowerCase().includes(repoSearch.toLowerCase())).map(r=>(
            <div key={r.id} className="flex items-center justify-between gap-3 rounded-xl border border-[#18181b] px-3 py-2.5 hover:border-brand-300 hover:bg-brand-50/40 transition-colors">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-navy-900 truncate">{r.name}</span>
                  <span className={`chip text-[10px] ${r.private?'bg-amber-50 text-amber-700 border border-amber-200':'bg-gray-100 text-gray-500 border border-[#18181b]'}`}>{r.private?'private':'public'}</span>
                </div>
