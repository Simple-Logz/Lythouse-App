...
const RISK_CLS: Record<string,string> = {
  ...
  unknown:'bg-gray-100 text-gray-500 border border-[#18181b]',
};
const STATUS_CLS: Record<string,string> = {
  ...
  cancelled:'bg-gray-100 text-gray-500 border border-[#18181b]',
};
...
function NotGenerated({onGenerate,loading,label}:{onGenerate:()=>void;loading:boolean;label:string}){
  return<div className="flex items-center justify-between gap-3 rounded-xl border border-dashed border-[#18181b] bg-gray-50 px-3.5 py-3">
      ...
      items.length?(
        <div className="space-y-2.5">
          {items.map((it:any,i:number)=>(
            <div key={i} className="rounded-xl border border-[#18181b] px-3.5 py-2.5">
...
function ChecklistRow({ok,warn,label,detail}:{ok:boolean;warn?:boolean;label:string;detail?:string}){
  return<div className="flex items-start gap-2.5">
    {ok?<CheckCircle2 size={16} className="mt-0.5 shrink-0 text-green-600"/>:warn?<AlertTriangle size={16} className="mt-0.5 shrink-0 text-amber-500"/>:<span className="mt-0.5 h-4 w-4 shrink-0 rounded-full border-2 border-[#18181b]"/>}
    <div className="min-w-0 flex-1">
    ...
    <div className="relative">
      {stages.map((s,i)=>{
        ...
        return<div key={s.key} className="relative flex gap-3 pb-5 last:pb-0">
          ...
          <span className={`mt-0.5 flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full ${s.done?'bg-brand-500 text-white':'border-2 border-[#18181b] bg-white'}`}>{s.done&&<CheckCircle2 size={12}/>}</span>
          <div className="min-w-0 flex-1">
        ...
        {areas.length>0&&(
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {areas.map(([area,count])=><span key={area} className="chip bg-gray-100 text-gray-600 border border-[#18181b] font-mono text-2xs">{area}/ <span className="font-sans font-semibold">×{count}</span></span>)}
            </div>
          </div>
        )}
...
export function ProjectReadyCard({project,validation,openFindings,readinessPct,estWindow,onDraft,drafting}:{project:any;validation:any;openFindings:number;readinessPct:number|null;estWindow:string;onDraft:()=>void;drafting:boolean}){
  ...
  return<div className="group relative rounded-2xl border border-[#18181b] bg-white p-4 transition-all duration-200 hover:border-brand-200 hover:shadow-md">
