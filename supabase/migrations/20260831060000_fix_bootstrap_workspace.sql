-- Fix ambiguous OUT parameter references in bootstrap_user_workspace and keep workspace bootstrap tenant-safe.
...
declare
 v_user uuid:=auth.uid(); v_org uuid; v_ws uuid; v_name text; v_slug text;
begin
 if v_user is null then raise exception 'authentication required'; end if;
 select wm.workspace_id into v_ws from public.workspace_members wm where wm.user_id=v_user order by wm.created_at asc limit 1;
 if v_ws is not null then
   select w.organization_id into v_org from public.workspaces w where w.id=v_ws;
   organization_id:=v_org; workspace_id:=v_ws; return next; return;
 end if;
 select om.organization_id into v_org from public.organization_members om where om.user_id=v_user order by om.created_at asc limit 1;
 if v_org is null then select o.id into v_org from public.organizations o where o.owner_id=v_user order by o.created_at asc limit 1; end if;
 ...
 v_slug:=v_slug||'-'||substr(replace(v_user::text,'-',''),1,8);
 ...
 select w.id into v_ws from public.workspaces w where w.organization_id=v_org and w.owner_id=v_user order by w.created_at asc limit 1;
